...
class Blur {
	...
	public static Bitmap apply(Context context, Bitmap sentBitmap, int radius,
		...
		/**
		 * If you want to use renderScript algorithm, first check if build
		 * version is < JellyBean to use library compat.
		 */
		if (useRenderScript) {
			if (Build.VERSION.SDK_INT > Build.VERSION_CODES.JELLY_BEAN) {
				...
				try {
					final android.support.v8.renderscript.RenderScript rs = android.support.v8.renderscript.RenderScript
							.create(context);
					// use this constructor for best performance, because it
					// uses USAGE_SHARED mode which reuses memory
					final android.support.v8.renderscript.Allocation input = android.support.v8.renderscript.Allocation
							.createFromBitmap(rs, bitmap);
					final android.support.v8.renderscript.Allocation output = android.support.v8.renderscript.Allocation
					...
					final android.support.v8.renderscript.ScriptIntrinsicBlur script = android.support.v8.renderscript.ScriptIntrinsicBlur
							.create(rs, android.support.v8.renderscript.Element
									.U8_4(rs));
					script.setRadius(radius);
					script.setInput(input);
					script.forEach(output);
	 ...
	 * http://incubator.quasimondo.com created Feburary 29, 2004 Android port :
	 * Yahel Bouaziz <yahel at kayenko.com> http://www.kayenko.com ported april
	 * 5th, 2012
	 * 
